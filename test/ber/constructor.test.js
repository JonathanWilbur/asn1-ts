...
describe("The Basic Encoding Rules Constructor", () => {
    ...
    it("encodes another element correctly", () => {
        const inner = new asn1.BERElement(
            asn1.ASN1TagClass.universal,
            asn1.ASN1Construction.primitive,
            asn1.ASN1UniversalType.boolean,
            true
    ...
    it("encodes a Date correctly", () => {
        ...
        expect(el.generalizedTime.toISOString().slice(0, 8)).toBe((new Date(2019, 10, 3)).toISOString().slice(0, 8));
    });
});
